...
6. **Model host** — prefix is the host. This engine checkout and **`modelbasedPrj-*`** are **repo-based** (`sysml-models/` is model SSOT). **MUST NOT** use SysMLEdge as model SSOT on `modelbasedPrj-*`. **`SysMLEdgePrj-*`** and bound: the working model SSOT is hosted by SysMLEdge; MemNet stays shared working memory. Two `pin_map` tools MUST NOT substitute. Detail: `docs/application-notes/system/llm-system-dev-multitask.md`.
...
- **Tests / lint**: `pytest` (runs fully in-process; `tests/conftest.py` sets `MEMNET_TEST_INLINE=1`, no serve daemon needed). `ruff check` + `ruff format --check` over `parts/common/memnet parts/memnet-mcp/software tests`. One test (`agensgraph_live`) is skipped unless `MEMNET_AGENSGRAPH_URL` points at an external AgensGraph server. Neo4j is not a backend. `MEMNET_NEO4J_*` is ignored; the engine starts.
...
- **Tip access portal**: ops sidecar `ops/tip_access_portal` (`memnet-tip-portal`), not the Hatch wheel. Keyed tip MCP only (`https://memnet.139-59-255-181.nip.io/mcp`). Env placeholders in `ops/tip_access_portal/.env.example`. nginx `auth_request` teach: `docs/operations/tip-memnet-access-portal.md`. tip≠face.
- **SysML:** this checkout is **repo-based**. Map `schema.sysml.example.txt` (not the game map) or ingest fails `unknown_tag`. Path-B `ingest_sysml` is **1 path → current session**. **Model Snap** (`memnet snap model` / `snap_model`) is one load tree → catalog + interiors. Goldfish: relatives of **one** interior; look loop re-anchors `session=` (one \(S\) per generate). A big load tree (~10k pins) is Model Snap (MN-REQ-11.17): interiors of about 2\(M\) (\(M=50\)), one `pin_map` per generate. Do not set `ingest_sysml` `max_nodes` to the whole tree (`dumpModelIntoOneSession=false`). This repo already misses the default cap: `requirements.sysml` is 229 pins, `deploy.sysml` is 504, and `snap_model` at `max_nodes=200` raises `ingest_budget` before the 2\(M\) cut. The live split is one level. Cue a returned `qname=` / `requirementId=` (e.g. `REQ_MN_REQ_00`). leftover `anchor=` is leftover. **`modelbasedPrj-*`:** git `sysml-models/` is model SSOT; **MUST NOT** use SysMLEdge as model SSOT. **`SysMLEdgePrj-*`** and bound: MemNet stays campaign working memory; bound desk hosts the model graph (`docs/application-notes/system/llm-system-dev-multitask.md`).
