...
6. **Model host** — this engine checkout is **repo-based** (`sysml-models/` is model SSOT here). Downstream overlays: operator README states **repo-based** or **SysMLEdge-based**. When SysMLEdge-based and bound, the SSOT graph is hosted by SysMLEdge; MemNet stays shared working memory. Two `pin_map` tools MUST NOT substitute. Detail: `docs/application-notes/system/llm-system-dev-multitask.md`.
