...
- **Path-B PinMapIngest:** all domains shipped (`memnet.pin_map_ingest`; CLI/MCP `ingest sysml|codebase|pcba|skills`). **CatalogSnap** 0.15: `memnet snap model` / MCP `snap_model` + `session_list` (`sessions|n/max`) + `session_close` + `session_drop_stale` (idle / TTL-expired; dry-run unless apply). **PinMapExport** 0.19: cue `pin_map` GQL write-out (`memnet export pin-map`). Re-ingest / `.sysml` reverse (#66) not claimed.
