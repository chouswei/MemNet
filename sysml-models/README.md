...
- **Path-B PinMapIngest:** all domains shipped (`memnet.pin_map_ingest`; CLI/MCP `ingest sysml|codebase|pcba|skills`). **PinMapExport** 0.19: cue `pin_map` GQL write-out (`memnet export pin-map`). Re-ingest / `.sysml` reverse (#66) not claimed.
