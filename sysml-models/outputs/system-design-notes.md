...
3. **Durable GQL store behind MemNet** — M2.5 / **0.7** Agens live hydrate/flush (`DurableStoreAdapter` / Fake / optional AgensGraph client; optional Neo4j extra **0.14** `liveNeo4jClaimed=true`; extra **0.16** optional library database name on the same Neo4j process, locators only, name MUST differ from the cabinet (`rejectSameNameAsCabinet`); one sync owner). Extra **0.17** `RagHostHook.implemented=true` **outside** `MemNetSystem` (skip valid). Cabinets external / not vendored.
...
| Cousin pointing contrast | TARGET cue→RelativeSeed→ShapeWalk vs eight cousins (`CousinPointingContrast` in `models/cousins.sysml`; MN-REQ-02.9 / 04.8 / 11.17.1). Copy cue-without-store-key + neighbourhood emit. Do not copy engines, unique-name MERGE, silent LLM same-name merge, content-hash ids, typed path-ids, or vector indexes as identity. SysMLEdge is a distinct desk `pin_map` (not MemNet session strata; not MemNet SSOT; this repo has no product face). Overlay family is `SysMLEdgePrj-*` (prefix `SysMLEdgePrj-`); this engine checkout is **repo-based** (git `sysml-models/` is model SSOT here). Downstream **SysMLEdge-based** and bound: the desk graph is working model SSOT; MemNet stays mission working memory. |
...
| WRITE (CREATE / SET / DELETE) | `mutate` (leftover `add` / `update` named leftover) |
