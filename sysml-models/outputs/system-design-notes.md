...
3. **Durable GQL store behind MemNet** — M2.5 / **0.7** Agens live hydrate/flush (`DurableStoreAdapter` / Fake / optional AgensGraph client; optional Neo4j extra **0.14** `liveNeo4jClaimed=true`; extra **0.16** optional library database name on the same Neo4j process, locators only; one sync owner). Cabinets external / not vendored.
...
│                         + Neo4jLibraryPort          // 0.16 optional second DB; locators only
