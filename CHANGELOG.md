...
- **Honesty `c` — tip portal UX cut** — Invite-only `/` (Google button; no service table). `/status` uses Reachable/Unreachable (401 still up) with a non-colour mark; public omits Clients. One-time key page: Copy feedback, header snippet, empty state “already shown”. Admin: mint first, human columns, confirm on Revoke, scrollable tables. No Hatch bump.
- **Honesty `c` — `modelbasedPrj-*` MUST NOT use SysMLEdge as model SSOT** — Prefix is the host, not a README flip. `modelbasedPrj-*` and this engine checkout are **repo-based** (git `sysml-models/`). MUST NOT copy a SysMLEdge host onto that family. `SysMLEdgePrj-*` and bound: desk graph is working model SSOT; MemNet stays campaign goldfish. `CousinSysMLEdge`: `modelbasedPrjRepoBased=true`, `modelbasedPrjSysmlEdgeIsModelSsot=false`. No Hatch bump.
- **Honesty `c` — SysMLEdge users: MemNet is working memory; bound desk hosts the model graph** — Playbook host mode (`docs/LLM-GUIDE.md`, `docs/application-notes/system/llm-system-dev-multitask.md`). Prefix is the host. This engine checkout stays repo-based. Downstream `SysMLEdgePrj-*` bound: SSOT graph on the desk; MemNet stays shared campaign goldfish. Two `pin_map` tools MUST NOT substitute. `CousinSysMLEdge` honesty: `memnetIsMissionWorkingMemory`, `operatorReadmeStatesHost` (SysMLEdgePrj overlay states SysMLEdge-based; not a flip for `modelbasedPrj-*`), `downstreamBoundDeskIsWorkingModelSsot`. No Hatch bump.
- **Honesty `c` — tip portal service and client status look** — `ops/tip_access_portal` `/status` (and admin) probe configured MemNet listeners (`MEMNET_STATUS_PROBES` / `MEMNET_TIP_MCP_PROBE`; HTTP 401 still counts as up). Admin sees probe targets and Bearer last-used / call counts (`GET /auth/validate` records use). Look only: no restart, mutate, or `session_close`. `MemNetUsageDashboard.httpImplemented` stays `false` (`unparksUsageDashboard=false`). No Hatch bump.
- **Honesty `c` — tip MemNet access portal (sidecar)** — `ops/tip_access_portal` (`memnet-tip-portal`) implements `TipMemNetAccessPortal` outside `MemNetSystem` (`MN-REQ-06.8` / `MN-VER-06-S05`). Admin Szu-Wei mints invites; Google login; Bearer shown once (hash stored) for keyed tip MCP at `https://memnet.139-59-255-181.nip.io/mcp`; `GET /auth/validate` for nginx `auth_request`; unauthenticated WWW MemNet refused; forward to Pi tip. tip≠face. Not a sysmledge product and not an `openProject` key page. `portalWebImplemented=true`; `inventOnly=false`; `sellsInvent2Green=false`. No Hatch bump.
