...
### Changed
- **Session-in-session loop + parallel interiors** — Goldfish recurses **across generates** (one \(S\) per `pin_map`; child `session=` → drop map, re-anchor). Sibling sub-units MAY be parallel `TSK_*` once the parent shell is already in `.sysml` (disjoint interiors; TCP/HTTP). Case study Turn I. Package SemVer remains **0.9.0**. Do not claim **1.0**.
...
- **Refactor `llm-sysml-v2-modeling`** — One note: `.sysml` SSOT; MemNet shows **relatives** of one cut (\(\approx 1\)–\(2\,\mathrm{k}\) vs \(\approx 38\,\mathrm{k}\)–\(113\,\mathrm{k}\) dump). One turn loop. Complete Shape or refuse. VehicleUsages + PDU as worked relatives. Pointer to nest-cut case study. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **VehicleUsages nest review (OMG SysML v2 example)** — Official [VehicleUsages.sysml](https://github.com/Systems-Modeling/SysML-v2-Release/blob/master/sysml/src/examples/Vehicle%20Example/VehicleUsages.sysml): defs vs usages as two interiors of **one** Snap; `subsets`/`redefines` = configuration delta not a pasted nest; multiplicity is one pin; `interface` to nested ports (`vehicle_C3` drive shaft) = second look / Absorb slice. Ingest leftover: no `interface` usage / `connect` / `flow` in `_DEF_HEAD`. Case study Turn F. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **SysML application notes — nest cuts, no truncated Shape** — [`docs/application-notes/llm-sysml-v2-modeling.md`](docs/application-notes/llm-sysml-v2-modeling.md) §6: SysML can nest everything; sessions cut the containment tree so each interior **fits** goldfish \(M\) whole. Silent `max_rows` / shell drop / ingest mid-brace are not Shape (same class as silentPickOneRoot). Package interiors remain a convenient first cut; recurse part-in-part. Nested usage is a locator, not a copied nest. Worked examples and case study: [`sysml-models/outputs/sysml-session-nest-cuts-case-study.md`](sysml-models/outputs/sysml-session-nest-cuts-case-study.md) (requirement / part / package / satisfy / view; TARGET complete Shape vs leftover `max_rows`). Multitask note distinguishes `ingest_sysml` 1→1 from `snap_model`. Grammar pointer: [`docs/grammar/memnet-session-strata.md`](docs/grammar/memnet-session-strata.md). As-is leftover: engine `context_pack` still caps the walk; do not teach that as law. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **Façade leftover honesty (2026-08-20 gap scan)** — Product write is MCP/CLI `mutate` (GQL Commit: CREATE/MERGE/SET/DELETE; no NEW mint). leftover `add`/`update` remain leftover-named (Path-B pipe seeds still use leftover `add`; they do not mint NEW). leftover CLI `read get` unshipped from the product CLI (`leftoverReadGetNested=false`); MCP still has no `read_get`. Product `pin_map` teach/schema is cue / codebook tokens / session; leftover `--anchor` / `anchor=` named leftover. Empty q is 0.11 outline **regardless of view**; `view=shell` is grain on a seed. leftover `import-slice --id-policy` named leftover (keep = leftover MERGE-as-lookup). leftover MCP `query_walk` and CLI `query neighbors|path|context|walk` named leftover; `query context` is not `require_anchor` as product law. No `rag_query`. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **Example docs vs leftover dialect** — Bundled `parts/common/memnet/memnet/examples/` README and headers no longer teach Tier A / Layer / `+/~/-` mutate. Coding seed locators match 0.9.0 (`cli.py` line numbers, MCP `session_load`/`save`). InvAmp math example dropped retired Layer ASCII (archive remains `docs/grammar/archive/`). `memnet examples add` samples are GQL CREATE. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **Docs / nest honesty** — `system-design-notes.md`: CueConflict `implemented=true`; extras 0.10–0.19 on master; pin_map default goldfish from a cue (not “when leftover-anchored”). `leftover_empty_seed_skip` doc: engine now outlines empty q (do not flip `SessionOutline.implemented` or `SameThingAbsorb.implemented`). Skill: PyPI `memnet-llm` is **0.4.6**, Hatch **0.9.0**. Playbook + application notes: leftover NEW / `--anchor` / leftover `add` named leftover; GQL Commit via `mutate`.

### Added
- **GraphGlot parse front** — [#109](https://github.com/chouswei/MemNet/issues/109) @ 73a63c9b. Accept parse calls PyPI `graphglot>=0.14.2` (Apache-2.0; [`NOTICE`](NOTICE)); MemNet product gate still rejects Layer and free WITH / UNWIND / CALL / unbounded MATCH…RETURN even when GraphGlot parses them. Not a second accept dialect and not a store. No TCK claim. Grammar spelling SSOT remains vendored [`docs/grammar/openCypher.bnf`](docs/grammar/openCypher.bnf). leftover `by_id` / `id_first` / `NEW` stay leftover honesty. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **0.18 Peak_L last-resort (untagged)** — Topology cue on residual \(\rho^*\) (incident edges except hierarchical `contains`; hide recycled) when codebook tokens miss MATCH_L. Still one Recall: RelativeSeed → ShapeWalk → `pin_map`. Peak is a cue, not a third operator. Empty \(q\) stays 0.11 outline (Peak SHALL NOT hijack). Peak is **not** default goldfish (`defaultGoldfish=false`). Two peaks emit CueConflict (SHALL NOT absorb in Recall; SHALL NOT silent MERGE-by-name). V9 paradox pytest: contains-parent raw degree is not \(\rho^*\). Hid stays off the wire. leftover `--anchor` / NEW stay leftover. `operatorCount` stays 2. `liveNeo4jClaimed` stays **true**. `RagHostHook.implemented` stays **true** (extra **0.17**, outside `MemNetSystem`). No `rag_query`. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **0.17 HostSearch locators (untagged)** — `RagHostHook.implemented=true` **outside** `MemNetSystem` (`memnet.rag_host_hook`). Cue → locators or skip (fail-open). Optional `MEMNET_HOST_SEARCH_URL` (do not vendor a RAG server); MAY consume extra **0.16** library locators. Commit via existing MutateGate / Path-B ingest. MUST NOT absorb. MUST NOT `rag_query` MCP. MUST NOT Snap-on-session. MUST NOT fuse RRF / PPR / Leiden into `pin_map`. Hid / generate / chunk bodies stay off the emit. leftover ingest `allocate_from_locator`-as-PK stays leftover. `hostSearchInsideSystem` stays false. `liveNeo4jClaimed` stays **true**. Cousin nest stays false. `operatorCount` stays 2. Package SemVer remains **0.9.0**. Do not claim **1.0**. Peak_L is extra **0.18**.
- **0.16 two Neo4j namespaces (untagged)** — same Neo4j process; cabinet database (`MEMNET_NEO4J_DATABASE`, default `neo4j`) vs optional library database (`MEMNET_NEO4J_LIBRARY_DATABASE`). Skip the library bind when the name is unset. Library port emits **locators only** (`generate=false`). MUST NOT hydrate \(S\) from the library database; MUST NOT flush session pins into it. Two cabinet **URLs** still require `MEMNET_DURABLE_BACKEND`. MUST NOT fuse RRF / PPR / Leiden / ANN into `pin_map`. `RagHostHook.implemented` stays **false** (0.17). No `rag_query` MCP. No Snap-on-session. Server not vendored. LLM MUST NOT talk Bolt. Hid / elementId / `_memnet_hid` stay off CLI/MCP/GQL emit. `liveNeo4jClaimed` stays **true**. Cousin nest `liveNeo4jClaimed` stays false. `operatorCount` stays 2. Package SemVer remains **0.9.0**. Do not claim **1.0**.
- **0.14 Live Neo4j claimed (untagged)** — operator proof on rpi5-syson @ d23cc71 (`/home/instmeasure/venvs/memnet`; version string still 0.9.0). `merge_pattern: hid`; `flush_cypher_has_hid_merge: yes`; `live_round_trip: yes`. Nodes `COM_acme_live_neo4j`, `TSK_mission_q3`; edge `(TSK_mission_q3)-[:ABOUT]->(COM_acme_live_neo4j)` `E_about_q3`. Flush MERGE used `{_memnet_hid: $hid}`. Hydrate first MATCH by hid got zero rows (cabinet hid ≠ leftover nickname); leftover-nickname hydrate after hid miss returned the ego + ABOUT. `hydrate_match_hid: no`. Honest note: live round-trip yes; hid flush; leftover-nickname hydrate after hid miss. Do **not** write hydrate-by-hid proven on live. `liveNeo4jClaimed=true`. Package SemVer remains **0.9.0**. Server not vendored. Skip live pytest unless `MEMNET_NEO4J_URL`. Hid stays off CLI/MCP/GQL emit. Do not claim **1.0**. InvenTree/droplet untouched. Not a close of [#65](https://github.com/chouswei/MemNet/issues/65).
- **0.19 pin-map export (untagged)** — CLI `memnet export pin-map` / MCP `export_pin_map` writes out a cue `pin_map` as shaped GQL (empty q = 0.11 outline, not a dump of \(S\)). Hard bounds stay. CueConflict if \(|Q|>1\). Ingest is not export. Distinct from session snapshot and from ImportAbsorb / `export_working_memory_slice`. Chat dump is not export. leftover ids are not law; hidden handle stays off the wire. MUST NOT MERGE-by-id on the way out; MUST NOT `rag_query`. Re-ingest / identity merge (MN-REQ-11.5 SHOULD) and `.sysml` file generation remain a later path. Ingest no longer satisfies MN-REQ-11.1–11.5. Package SemVer remains **0.9.0**. Do not claim **1.0**. `liveNeo4jClaimed` stays **true** (0.14; this extra does not own the Neo4j claim).
- **0.15 catalog Snap + session strata + model Snap (untagged)** — `Snap(model)` mints a catalog session plus package interiors (kind band / child package only when an interior exceeds about 2\(M\)). Catalog pins carry `session=` + `qname=` locators (properties). List those ids; look = `pin_map`; join = Path-B Absorb of a **slice**. CLI `memnet snap model`; MCP `snap_model` / `session_list`. ImportAbsorb stays pattern match; ingest locators stay properties. Hid stays off the wire. Package SemVer remains **0.9.0**. Do not claim **1.0**. Catalog cut's own fence: this extra does not own `liveNeo4jClaimed` (product claim stays **true** from 0.14). **MUST NOT:** ANN-rank sessions; Absorb a whole \(S\); dump one model into one session; revive Layer; one session per TSK or per REQ; invent a store key.
- **0.11 session outline (untagged)** — Empty cue is Recall of \(S\): kinds that exist plus LIMIT exemplars with actual labels and properties (`## outline`), **regardless of `view`**. Dark session joins the handed session id and asks the session, not a node. Still one Recall operator. `view=shell` is grain on a seed — **not** the outline. CueConflict if exemplars collide on a name cue (two roots stay two). leftover empty-seed skip is leftover only. Hidden handle stays off the wire. SameThingAbsorb not this cut. Package SemVer remains **0.9.0**. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this outline cut). SysML nest `#118` stays `SessionOutline.implemented=false` (engine honesty; no nest fight).
- **0.13 goldfish caller contract (untagged)** — Playbook + MCP/CLI: each turn `pin_map(q)` (or empty-q outline); **drop** prior map rows from the prompt; sparse Δ; env blobs stay in the harness. Pytest: a stuffed history of maps is a fail (`stuffed_maps`). `view=shell` remains grain on a seed (not 0.11 session outline). leftover `--anchor` is leftover, not TARGET law. Sibling user-pack may absorb caller text later; this repo owns the fail. Package SemVer remains **0.9.0**. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this caller cut).

### Changed
- **0.12 SameThingAbsorb (untagged)** — Commit rule after CueConflict: `MATCH (a:Label {…}), (b:Label {…}) SET a += b` collapses two live pattern nodes (labels+properties; `aka` on the survivor). Agent-gated; find/`pin_map`/MERGE when \(|Q|>1\) still emit CueConflict and do not absorb. Distinct from ImportAbsorb (Path-B slice; SHALL NOT entity-resolve). Hidden handle stays off the wire. Not a product verb. SysML `SameThingAbsorb.implemented` stays **false** (Sysmler nest pin). Empty-q outline (0.11) SHALL NOT absorb. Package SemVer remains **0.9.0**. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this absorb cut).

- **0.10 identity cut (untagged)** — GraphElement is identity; hidden handle off the wire; optional `id` is a nickname. `CREATE ()` is legal. MERGE/SET/DELETE by labels+properties. `pin_map` from cue; CueConflict on find/`pin_map` emit when \(|Q|>1\). SCHEMA is not `id_first`. leftover NEW mint, leftover `require_anchor`, leftover `read_get`, ingest `allocate_from_locator` as PK, import MERGE-by-id, and leftover `id_policy keep|reject|remint` as product commands stay leftover (not taught). Package SemVer remains **0.9.0**. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this identity cut). Neo4j adapter flush uses hidden-handle pattern MERGE (not `{id}`).
