...
### Changed
- **Honesty `c` — `session save` after TTL expiry** — Explicit `session save --file` writes an expired in-RAM session (then drops the id; `@WRN: session_expired_saved`). `pin_map` / mutate still `session_expired`. Optional `MEMNET_EXPIRE_SNAPSHOT_DIR` auto-writes `{session_id}.snap` on purge/get expire. Not Neo4j; not a whole-store cabinet dump. Same goldfish loop. No Hatch bump.
