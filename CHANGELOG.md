...
### Changed
- **Honesty `c` — cross-cut `satisfies` on catalog Snap** — `snap_model` projected each package alone, so `satisfy PkgReq::ReqAlpha` in another interior was a **silent miss** (`_edges_in` keeps only both-ends-in-slice; per-file ingest index never sees the dest). Catalog now commits locator stubs (`session=` + `qname=` / `requirementId=`, grain `cross_cut`) and a `:satisfies` edge when unique ends fit goldfish \(M\); otherwise a package-pair `:satisfies` plus `@WRN: cross_cut`. Interiors MUST NOT grow a dangling dest node. Unresolved target: `@WRN: cross_cut_miss`. Not a join-across-sessions protocol; not Absorb of a whole \(S\); not N-server (#47). Same goldfish loop (`cue → pin_map` catalog, then one interior). Model: **MN-REQ-11.17.1** + `CatalogSnap` / `CmdCatalogSnap` flags; SysMLEdge is a cousin (`CousinSysMLEdge`) — distinct `pin_map`, not SSOT, no product face in this repo. Overlay family is **`SysMLEdgePrj-*`** (literal prefix `SysMLEdgePrj-`; older two-store family remains `modelbasedPrj-*`); git `sysml-models/` stays structural SSOT in both. No Hatch bump. See [`docs/extras/memnet-session-strata.md`](docs/extras/memnet-session-strata.md); regression: `tests/test_catalog_snap.py`.
