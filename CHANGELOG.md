...
- **Cabinet hydrate MATCH by hid** — Neo4j / AgensGraph flush MERGE and hydrate ego MATCH share `_memnet_hid`. ABOUT / edges MATCH the same COM hid (not leftover nickname `COM_acme_live_neo4j` as the cabinet key). Official fixture flush then hydrate by the **same hid** that was written. leftover `id` is a property filter after hid miss only. Tests assert hid MERGE, not `MERGE (n:COM {id:`. Do **not** claim live Neo4j (`liveNeo4jClaimed` stays false; 0.14 still unclaimed). Package SemVer remains **0.9.0**. Hidden handle stays off CLI/MCP/GQL emit.
