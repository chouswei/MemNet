...
### Changed
- **RAG / Neo4j rethink (design)** — [`docs/grammar/memnet-neo4j-rag-rethink.md`](docs/grammar/memnet-neo4j-rag-rethink.md): two ports on one Neo4j; **order-of-magnitude estimates** (Bolt RTT, `pin_map` tens of KiB / tens of MiB \(S\), flush ≤150 auto-commit MERGEs, library GiB not in goldfish, LLM-bound generate vs ms Shape). Does not amend SHAPE; does not ship HostSearch.
