...
### Changed
- **Package identity 0.19.0** — Hatch / `project.toml` / `memnet.__version__` catch up with numbered extras **0.10–0.19** (identity through pin-map export). Git tag `v0.19.0` remains coordinator. PyPI `memnet-llm` is still **0.4.6**. **1.0** stays a claim of 0.5–0.8 (this cut is not 1.0).
...
- **Docs / nest honesty** — `system-design-notes.md`: CueConflict `implemented=true`; extras 0.10–0.19 in package 0.19.0; pin_map default goldfish from a cue (not “when leftover-anchored”). `leftover_empty_seed_skip` doc: engine now outlines empty q (do not flip `SessionOutline.implemented` or `SameThingAbsorb.implemented`). Skill: PyPI `memnet-llm` is **0.4.6**, Hatch **0.19.0**. Playbook + application notes: leftover NEW / `--anchor` / leftover `add` named leftover; GQL Commit via `mutate`.
