...
- **Sessions as strata (design)** — partition haystacks into named sessions; goldfish one \(S\); join Absorb a slice. **SysML:** Snap each `models/*.sysml` into its own library session; catalog of `session=` + `path=`; mission keeps `TSK`/`MOD` locators. Not Layer. [`docs/grammar/memnet-session-strata.md`](docs/grammar/memnet-session-strata.md).
