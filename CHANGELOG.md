...
### Added
- **Product shape (from the problem)** — [`docs/SHAPE.md`](docs/SHAPE.md): why chat / dump-\(S\) / RAG fail; necessary form (session, goldfish \(\tilde{X}\), gated \(\Delta\), two budgets); distinct from Recall Shape. SysML: application notes `llm-sysml-v2-modeling.md` / `llm-system-dev-multitask.md` vs product `sysml-models/`. Versions fill the shape; 1.0.0 is that shape mature for people.
...
### Changed
- **Multitask ops (as-is 0.4.x)** — [`docs/multi-agent-sessions.md`](docs/multi-agent-sessions.md): product shape; cue then `pin_map`; MCP `session`; RSV + Path-B ingest **shipped**; CapsPolicy when ACL on; full ACL modes still to-be. Hub `AGENTS.md` / `.cursor/rules/memnet-multitask.mdc` no longer call ingest/reserve design-only.
...
- **Roadmap 0.5** — superseded by [`docs/ROADMAP.md`](docs/ROADMAP.md). ACL / RSV / ingest are 0.4.x, not deferred.
