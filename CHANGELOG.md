...
### Changed
- **Honesty `c` — device MemNet services, one MCP at the droplet** — Ops fleet modelled outside `MemNetSystem`: `MemNetDeviceHost[1..*]` run `MemNetCoreLibrary` (no product MCP); `DropletHost` hosts the one `MemNetMcpServer`. `MN-REQ-06.6` / `MN-VER-06-S03`. Not N-server federation (#47). Same goldfish loop. No Hatch bump. See [`sysml-models/outputs/device-fleet-one-mcp-case-study.md`](sysml-models/outputs/device-fleet-one-mcp-case-study.md).
