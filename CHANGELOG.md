...
### Changed
- **MemNet ↔ Neo4j talk** — expand [`docs/grammar/neo4j-buffer.md`](docs/grammar/neo4j-buffer.md): what talks to what (GQL goldfish vs Bolt cabinet), **key features on both sides** (Recall/Commit/session vs Bolt/Cypher/MERGE auto-commit), same seam / different Cypher, steal grain / reject GraphRAG and LLM↔Bolt, honest `liveNeo4jClaimed=false`. Pointers from `SHAPE.md` §5 and the docs index.
