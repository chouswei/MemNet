...
- **Honesty `c` — SysMLEdge users: MemNet is working memory; bound desk hosts the model graph** — Playbook host mode (`docs/LLM-GUIDE.md`, `docs/application-notes/system/llm-system-dev-multitask.md`). Operator README states **repo-based** or **SysMLEdge-based**. This engine checkout stays repo-based. Downstream bound SysMLEdge: SSOT graph on the desk; MemNet stays shared campaign goldfish. Two `pin_map` tools MUST NOT substitute. `CousinSysMLEdge` honesty: `memnetIsMissionWorkingMemory`, `operatorReadmeStatesHost`, `downstreamBoundDeskIsWorkingModelSsot`. No Hatch bump.
