...
### Changed
- **Honesty `c` — tip portal UX cut (#179)** — Invite-only `/` (Google button; no service table). `/status` uses Reachable/Unreachable (401 still up) with a non-colour mark; public omits Clients. One-time key page: Copy feedback, header snippet, empty state “already shown”. Admin: mint first, human columns, confirm on Revoke, scrollable tables.
- **Honesty `c` — `modelbasedPrj-*` MUST NOT use SysMLEdge as model SSOT (#178)** — Prefix is the host, not a README flip. `modelbasedPrj-*` and this engine checkout are **repo-based** (git `sysml-models/`). MUST NOT copy a SysMLEdge host onto that family. `SysMLEdgePrj-*` and bound: desk graph is working model SSOT; MemNet stays campaign goldfish. `CousinSysMLEdge`: `modelbasedPrjRepoBased=true`, `modelbasedPrjSysmlEdgeIsModelSsot=false`.
- **Honesty `c` — SysMLEdge users: MemNet is working memory; bound desk hosts the model graph (#176)** — Playbook host mode (`docs/LLM-GUIDE.md`, `docs/application-notes/system/llm-system-dev-multitask.md`). Prefix is the host. This engine checkout stays repo-based. Downstream `SysMLEdgePrj-*` bound: SSOT graph on the desk; MemNet stays shared campaign goldfish. Two `pin_map` tools MUST NOT substitute. `CousinSysMLEdge` honesty: `memnetIsMissionWorkingMemory`, `operatorReadmeStatesHost` (SysMLEdgePrj overlay states SysMLEdge-based; not a flip for `modelbasedPrj-*`), `downstreamBoundDeskIsWorkingModelSsot`.
- **Honesty `c` — tip portal service and client status look (#177)** — `ops/tip_access_portal` `/status` (and admin) probe configured MemNet listeners (`MEMNET_STATUS_PROBES` / `MEMNET_TIP_MCP_PROBE`; HTTP 401 still counts as up). Admin sees probe targets and Bearer last-used / call counts (`GET /auth/validate` records use). Look only: no restart, mutate, or `session_close`. `MemNetUsageDashboard.httpImplemented` stays `false` (`unparksUsageDashboard=false`).
- **Honesty `c` — tip MemNet access portal (sidecar) (#175)** — `ops/tip_access_portal` (`memnet-tip-portal`) implements `TipMemNetAccessPortal` outside `MemNetSystem` (`MN-REQ-06.8` / `MN-VER-06-S05`). Admin Szu-Wei mints invites; Google login; Bearer shown once (hash stored) for keyed tip MCP at `https://memnet.139-59-255-181.nip.io/mcp`; `GET /auth/validate` for nginx `auth_request`; unauthenticated WWW MemNet refused; forward to Pi tip. tip≠face. Not a sysmledge product and not an `openProject` key page. `portalWebImplemented=true`; `inventOnly=false`; `sellsInvent2Green=false`.
- **Honesty `c` — tip MemNet access portal (invent) (#174)** — `TipMemNetAccessPortal` outside `MemNetSystem` (`MN-REQ-06.8` / `MN-VER-06-S05`). Admin Szu-Wei mints invites; Google login; Bearer for keyed tip MCP at `https://memnet.139-59-255-181.nip.io/mcp`; unauthenticated WWW MemNet refused; forward to Pi tip. tip≠face. Not a sysmledge product and not an `openProject` key page. Invent only (`portalWebImplemented=false`).
- **Package identity 0.19.13** — Hatch / `project.toml` / `memnet.__version__` honesty cut on **0.19** so #174–#179 tip can upload (0.19.12 already on git tag). Last published PyPI remains **`memnet-llm==0.19.6`** until upload.

## [0.19.12] - 2026-09-21

### Changed
- **Honesty `c` — skills: SSOT→code tracker; drop drifting code map (#171)** — Add `.cursor/skills/sysml-ssot-to-code/` (implementation ledger). Drop the duplicate `memnet-reference` code-map table (stale `cli/` path). Drop pack-only `sysml-allocate-generator` / hardware-generator teach from the relatives cache map (those folders are not vendored here). Route from `SKILL-GRAPH.md` / `AGENTS.md`. Same goldfish loop.
- **Honesty `c` — SSOT parts allocate to live code (one wheel, many hosts) (#171)** — `SoftwareAllocate` + `ImplementationTracker` (`MN-REQ-06.7` / `MN-VER-06-S04`). One Hatch `memnet-llm` wheel; MemNet MCP tip/ops (`tipIsFace=false`); cousin `sysmledge` not in the wheel (`mustNotInventUploadBind`). Not N-server (#47).
- **Honesty `c` — device MemNet services, one MemNet MCP at the droplet (tip≠face) (#170)** — `MemNetOpsFleet` outside `MemNetSystem`; one MemNet tip/ops MCP at droplet; Pi tip MCP allowed for ops; product invent face = `sysmledge`. Dual-MCP remeter: product Q → `sysmledge`. `MN-REQ-06.6` / `MN-VER-06-S03`. Not #47.
- **Package identity 0.19.12** — Hatch / `project.toml` / `memnet.__version__` honesty cut on **0.19** so #170+#171 tip can upload (0.19.11 already on PyPI).

- **Honesty `c` — configurable `session save` on TTL expiry** — Off unless ``MEMNET_SAVE_ON_EXPIRE``. Then explicit `session save --file` writes an expired in-RAM session (drops the id; `@WRN: session_expired_saved`). ``MEMNET_EXPIRE_SNAPSHOT_DIR`` auto-writes `{session_id}.snap` on purge/get expire. Default off: `session_expired` as before. Not Neo4j. Same goldfish loop. No Hatch bump.
- **Honesty `c` — SysML expire-save + nest review** — `SnapshotStore` / `CapsPolicy` / `CmdSessionSave` / `CmdSessionLoad` / `EvExpireTtl` match configurable TTL file save (default off; RAM drop; file until user drop; load restores; not Neo4j). `MN-VER-01-S03`. Leftover archive still off `ProjectMemNet` load; dashboard still look-only. No Hatch bump.
- **Honesty `c` — leftover archive off ProjectMemNet load** — leftover_* / TierACodec / LegacyPipe* honesty moved to `sysml-models/models/archive.sysml` (`MemNetArchive`). `config.yaml` omits that file; `root.sysml` MUST NOT import it. Product nest uses leftover flags only (not leftover_* parts under Schema / GraphStore). One-page nest: [`sysml-models/outputs/product-nest-one-page.md`](sysml-models/outputs/product-nest-one-page.md). Teach path stays GQL; dashboard remains look-only (`httpImplemented=false`, `tipIsFace=false`). Same goldfish loop. No Hatch bump.
- **Honesty `c` — ImportGuard optional-soft teach** — Guidebook MUST teach Path-B `import_slice` / ImportAbsorb **without** an LLM key; **CheapLlmImportGuard** is user-selectable (set `MEMNET_IMPORT_GUARD_API_KEY` on serve to activate; leave unset = off; optional `BASE_URL` / `MODEL`; `--no-guard` one-shot skip). Soft review is not SSOT; hard gates always run after; transport failures soft-skip. Not `MEMNET_MCP_HTTP_TOKEN` / `session_token` / RSV `llm_id`. Feature stays — do not deprecate. No Hatch bump. See [`docs/LLM-GUIDE.md`](docs/LLM-GUIDE.md).
- **Honesty `c` — unsync checkpoint playbook** — Teach-only coordinator doctrine (roles, Architect I/O, Bind atoms, checkpoint kinds, soft-pass kills, aging-safe model policy). Cites [`docs/operations/multi-agent-sessions.md`](docs/operations/multi-agent-sessions.md), [`docs/application-notes/system/llm-system-dev-multitask.md`](docs/application-notes/system/llm-system-dev-multitask.md), and vendored [`memnet-multitask`](.cursor/skills/memnet-multitask/). Does **not** fork transport or parent-worker MUST. No Hatch bump. See [`docs/application-notes/system/llm-unsync-checkpoint-pipeline.md`](docs/application-notes/system/llm-unsync-checkpoint-pipeline.md).
...
### Changed
- **Honesty `c` — catalog cross-session satisfy locators (#168)** — Tip SemVer so [#168](https://github.com/chouswei/MemNet/pull/168) can upload; PyPI already has **0.19.10** (2026-09-12). Catalog Snap names cross-cut `satisfies` with locator stubs (`session=` + `qname=` / `requirementId=`). Interiors MUST NOT grow a dangling dest. `CousinSysMLEdge` records `mustNotInventUploadBind=true` (distinct `pin_map`, not SSOT, no product face in this repo). Same goldfish loop. Numbered extras **0.10–0.19** unchanged. No 0.20 extra. Hatch **0.19.11**; last published PyPI remains **`memnet-llm==0.19.6`** until upload. Do not claim tip=face.
- **Package identity 0.19.11** — Hatch / `project.toml` / `memnet.__version__` honesty cut on **0.19**. Hatch is **0.19.11**; last published PyPI remains **0.19.6** until upload.
...
### Changed
- **Honesty `c` — store-identity keys off shaped emit** — Audit after [#148](https://github.com/chouswei/MemNet/pull/148): nickname `id` was already off `pin_map`, but `DROP_KEYS` was not a real strip. `hid` / `_memnet_hid` / `elementId` on `Record.fields` could still appear on GQL emit. SSOT is `SHAPE_DROP_KEYS` in `memnet.models`, applied in `_emit_props` (mutate ack and shaped present) and jsonl. Nickname `id` stays off **shaped read** only. RSV product errors no longer print hid. Ranking excludes `elementId` as well as hid / nickname. Package **0.19.5** patch on **0.19**. Numbered extras **0.10–0.19** unchanged. No 0.20 extra. `operatorCount` stays 2. Published as **`memnet-llm==0.19.5`**. Do not dump \(S\). Do not invent `rag_query`. **1.0** stays unclaimed. Surfaces: [`docs/operations/honesty-c-wire-audit.md`](docs/operations/honesty-c-wire-audit.md).
- **Package identity 0.19.5** — Hatch / `project.toml` / `memnet.__version__` honesty cut on **0.19**. Hatch and PyPI are both **0.19.5**; 0.19.5 honesty `c` (`SHAPE_DROP_KEYS`) is published.
...
### Removed
- **Stale LLM-noise docs** — Dropped Layer/Tier A archive, line-codec goldens under `docs/grammar/examples/`, historical pointers (`ROADMAP-0.5.md`, `gql-model-exam.md`), leftover design theses (harness / RAG relatives / Neo4j rethink / field-formulas / grammar-design / ANTLR notes), leftover `NEW` mint and novel-cut RPG case studies, and ignored archive pytest. Agent teach is GQL only (`docs/grammar/gql-wire-profile.md`). Engine leftover codecs stay in `memnet.tier_a` / `memnet.layer` (rejected on mutate). Do not restore those files as doctrine. [#139](https://github.com/chouswei/MemNet/pull/139).

### Changed
- **Package identity 0.19.1** — Hatch / `project.toml` / `memnet.__version__` patch on **0.19**. Numbered extras **0.10–0.19** are unchanged (no new extra row). **1.0** stays a claim of 0.5–0.8. Last published PyPI wheel remains **`memnet-llm==0.19.0`** until `scripts/publish.sh --upload` (this environment has no Twine token). Git tag `v0.19.1` after merge.
- **Docs layout (job folders)** — Live docs sit under `docs/grammar/` (GQL wire + math), `docs/cabinet/` (durable adapters), `docs/extras/` (numbered extras / ACL design), `docs/operations/` (Multitask), `docs/application-notes/{system,domains,examples}/`. Index load order: [`docs/README.md`](docs/README.md). Historical changelog rows keep old paths. [#140](https://github.com/chouswei/MemNet/pull/140).
- **Skills: GraphElement cue, not leftover `id`** — In-repo skills cue `pin_map` with `kind` + locators on `goal` / `name` / `qname` / `path` / `requirementId`. MATCH/SET the same. leftover `locators=['id=…']`, `MATCH ({id})`, `id:'NEW'`, and `+ TSK [NEW]` named leftover. Do not claim **1.0**. [#137](https://github.com/chouswei/MemNet/pull/137).
...
### Changed
- **Package identity 0.19.0** — Hatch / `project.toml` / `memnet.__version__` catch up with numbered extras **0.10–0.19** (identity through pin-map export). Git tag `v0.19.0` remains coordinator. PyPI **`memnet-llm==0.19.0`** is published (wheel + sdist, 2026-08-20). **1.0** stays a claim of 0.5–0.8 (this cut is not 1.0).
- **memnet-nested-sessions** — Look loop, already-built interiors, parallel `TSK_*` when the parent shell is clear. [`.cursor/skills/memnet-nested-sessions/`](.cursor/skills/memnet-nested-sessions/). Do not claim **1.0**.
- **memnet-use (how to use MemNet)** — In-repo hub skill [`.cursor/skills/memnet-use/`](.cursor/skills/memnet-use/): goldfish loop; specialists `mcp-memnet` / `memnet-format` / `memnet-multitask`. Distinct from `memnet-reference` (build this product). Do not claim **1.0**.
- **MemNet application skills in-repo** — Vendor `mcp-memnet`, `memnet-format`, `memnet-multitask`, `memnet-codebase-snap` under `.cursor/skills/` (from [cursor-user-skills](https://github.com/chouswei/cursor-user-skills); not the whole pack). Cloud VMs load these here. Doctrine SSOT remains `docs/`. Index: [`.cursor/skills/README.md`](.cursor/skills/README.md). Do not claim **1.0**.
- **SysML modelling note + nest-cut case study** — Note = loop (two token laws: relatives of one cue; sub-unit in a separate session; look loop one \(S\) per generate; parallel `TSK_*` when the parent shell is already in `.sysml`). Case study = evidence Turns A–I (product tree, VehicleUsages, elan8, already-built reuse, parallel interiors). As-is leftover: `max_rows` clip, package-grain Snap, `_DEF_HEAD` misses `interface`/`subsets`/`connect`. [`docs/application-notes/llm-sysml-v2-modeling.md`](docs/application-notes/llm-sysml-v2-modeling.md). Do not claim **1.0**.
- **Façade leftover honesty (2026-08-20 gap scan)** — Product write is MCP/CLI `mutate` (GQL Commit: CREATE/MERGE/SET/DELETE; no NEW mint). leftover `add`/`update` remain leftover-named (Path-B pipe seeds still use leftover `add`; they do not mint NEW). leftover CLI `read get` unshipped from the product CLI (`leftoverReadGetNested=false`); MCP still has no `read_get`. Product `pin_map` teach/schema is cue / codebook tokens / session; leftover `--anchor` / `anchor=` named leftover. Empty q is 0.11 outline **regardless of view**; `view=shell` is grain on a seed. leftover `import-slice --id-policy` named leftover (keep = leftover MERGE-as-lookup). leftover MCP `query_walk` and CLI `query neighbors|path|context|walk` named leftover; `query context` is not `require_anchor` as product law. No `rag_query`. Do not claim **1.0**.
- **Example docs vs leftover dialect** — Bundled `parts/common/memnet/memnet/examples/` README and headers no longer teach Tier A / Layer / `+/~/-` mutate. Coding seed locators match 0.9.0 (`cli.py` line numbers, MCP `session_load`/`save`). InvAmp math example dropped retired Layer ASCII (archive remains `docs/grammar/archive/`). `memnet examples add` samples are GQL CREATE. Do not claim **1.0**.
- **Docs / nest honesty** — `system-design-notes.md`: CueConflict `implemented=true`; extras 0.10–0.19 in package 0.19.0; pin_map default goldfish from a cue (not “when leftover-anchored”). `leftover_empty_seed_skip` doc: engine now outlines empty q (do not flip `SessionOutline.implemented` or `SameThingAbsorb.implemented`). Skill: PyPI `memnet-llm` is **0.19.0**, Hatch **0.19.0**. Playbook + application notes: leftover NEW / `--anchor` / leftover `add` named leftover; GQL Commit via `mutate`.

### Added
- **GraphGlot parse front** — [#109](https://github.com/chouswei/MemNet/issues/109) @ 73a63c9b. Accept parse calls PyPI `graphglot>=0.14.2` (Apache-2.0; [`NOTICE`](NOTICE)); MemNet product gate still rejects Layer and free WITH / UNWIND / CALL / unbounded MATCH…RETURN even when GraphGlot parses them. Not a second accept dialect and not a store. No TCK claim. Grammar spelling SSOT remains vendored [`docs/grammar/openCypher.bnf`](docs/grammar/openCypher.bnf). leftover `by_id` / `id_first` / `NEW` stay leftover honesty. Do not claim **1.0**.
- **0.18 Peak_L last-resort** — Topology cue on residual \(\rho^*\) (incident edges except hierarchical `contains`; hide recycled) when codebook tokens miss MATCH_L. Still one Recall: RelativeSeed → ShapeWalk → `pin_map`. Peak is a cue, not a third operator. Empty \(q\) stays 0.11 outline (Peak SHALL NOT hijack). Peak is **not** default goldfish (`defaultGoldfish=false`). Two peaks emit CueConflict (SHALL NOT absorb in Recall; SHALL NOT silent MERGE-by-name). V9 paradox pytest: contains-parent raw degree is not \(\rho^*\). Hid stays off the wire. leftover `--anchor` / NEW stay leftover. `operatorCount` stays 2. `liveNeo4jClaimed` stays **true**. `RagHostHook.implemented` stays **true** (extra **0.17**, outside `MemNetSystem`). No `rag_query`. Do not claim **1.0**.
- **0.17 HostSearch locators** — `RagHostHook.implemented=true` **outside** `MemNetSystem` (`memnet.rag_host_hook`). Cue → locators or skip (fail-open). Optional `MEMNET_HOST_SEARCH_URL` (do not vendor a RAG server); MAY consume extra **0.16** library locators. Commit via existing MutateGate / Path-B ingest. MUST NOT absorb. MUST NOT `rag_query` MCP. MUST NOT Snap-on-session. MUST NOT fuse RRF / PPR / Leiden into `pin_map`. Hid / generate / chunk bodies stay off the emit. leftover ingest `allocate_from_locator`-as-PK stays leftover. `hostSearchInsideSystem` stays false. `liveNeo4jClaimed` stays **true**. Cousin nest stays false. `operatorCount` stays 2. Do not claim **1.0**. Peak_L is extra **0.18**.
- **0.16 two Neo4j namespaces** — same Neo4j process; cabinet database (`MEMNET_NEO4J_DATABASE`, default `neo4j`) vs optional library database (`MEMNET_NEO4J_LIBRARY_DATABASE`). Skip the library bind when the name is unset. Library port emits **locators only** (`generate=false`). MUST NOT hydrate \(S\) from the library database; MUST NOT flush session pins into it. Two cabinet **URLs** still require `MEMNET_DURABLE_BACKEND`. MUST NOT fuse RRF / PPR / Leiden / ANN into `pin_map`. `RagHostHook.implemented` stays **false** (0.17). No `rag_query` MCP. No Snap-on-session. Server not vendored. LLM MUST NOT talk Bolt. Hid / elementId / `_memnet_hid` stay off CLI/MCP/GQL emit. `liveNeo4jClaimed` stays **true**. Cousin nest `liveNeo4jClaimed` stays false. `operatorCount` stays 2. Do not claim **1.0**.
- **0.14 Live Neo4j claimed** — operator proof on rpi5-syson @ d23cc71 (`/home/instmeasure/venvs/memnet`; version string still 0.9.0). `merge_pattern: hid`; `flush_cypher_has_hid_merge: yes`; `live_round_trip: yes`. Nodes `COM_acme_live_neo4j`, `TSK_mission_q3`; edge `(TSK_mission_q3)-[:ABOUT]->(COM_acme_live_neo4j)` `E_about_q3`. Flush MERGE used `{_memnet_hid: $hid}`. Hydrate first MATCH by hid got zero rows (cabinet hid ≠ leftover nickname); leftover-nickname hydrate after hid miss returned the ego + ABOUT. `hydrate_match_hid: no`. Honest note: live round-trip yes; hid flush; leftover-nickname hydrate after hid miss. Do **not** write hydrate-by-hid proven on live. `liveNeo4jClaimed=true`. Server not vendored. Skip live pytest unless `MEMNET_NEO4J_URL`. Hid stays off CLI/MCP/GQL emit. Do not claim **1.0**. InvenTree/droplet untouched. Not a close of [#65](https://github.com/chouswei/MemNet/issues/65).
- **0.19 pin-map export** — CLI `memnet export pin-map` / MCP `export_pin_map` writes out a cue `pin_map` as shaped GQL (empty q = 0.11 outline, not a dump of \(S\)). Hard bounds stay. CueConflict if \(|Q|>1\). Ingest is not export. Distinct from session snapshot and from ImportAbsorb / `export_working_memory_slice`. Chat dump is not export. leftover ids are not law; hidden handle stays off the wire. MUST NOT MERGE-by-id on the way out; MUST NOT `rag_query`. Re-ingest / identity merge (MN-REQ-11.5 SHOULD) and `.sysml` file generation remain a later path. Ingest no longer satisfies MN-REQ-11.1–11.5. Do not claim **1.0**. `liveNeo4jClaimed` stays **true** (0.14; this extra does not own the Neo4j claim).
- **0.15 catalog Snap + session strata + model Snap** — `Snap(model)` mints a catalog session plus package interiors (kind band / child package only when an interior exceeds about 2\(M\)). Catalog pins carry `session=` + `qname=` locators (properties). List those ids; look = `pin_map`; join = Path-B Absorb of a **slice**. CLI `memnet snap model`; MCP `snap_model` / `session_list`. ImportAbsorb stays pattern match; ingest locators stay properties. Hid stays off the wire. Do not claim **1.0**. Catalog cut's own fence: this extra does not own `liveNeo4jClaimed` (product claim stays **true** from 0.14). **MUST NOT:** ANN-rank sessions; Absorb a whole \(S\); dump one model into one session; revive Layer; one session per TSK or per REQ; invent a store key.
- **0.11 session outline** — Empty cue is Recall of \(S\): kinds that exist plus LIMIT exemplars with actual labels and properties (`## outline`), **regardless of `view`**. Dark session joins the handed session id and asks the session, not a node. Still one Recall operator. `view=shell` is grain on a seed — **not** the outline. CueConflict if exemplars collide on a name cue (two roots stay two). leftover empty-seed skip is leftover only. Hidden handle stays off the wire. SameThingAbsorb not this cut. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this outline cut). SysML nest `#118` stays `SessionOutline.implemented=false` (engine honesty; no nest fight).
- **0.13 goldfish caller contract** — Playbook + MCP/CLI: each turn `pin_map(q)` (or empty-q outline); **drop** prior map rows from the prompt; sparse Δ; env blobs stay in the harness. Pytest: a stuffed history of maps is a fail (`stuffed_maps`). `view=shell` remains grain on a seed (not 0.11 session outline). leftover `--anchor` is leftover, not TARGET law. Sibling user-pack may absorb caller text later; this repo owns the fail. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this caller cut).

### Changed
- **0.12 SameThingAbsorb** — Commit rule after CueConflict: `MATCH (a:Label {…}), (b:Label {…}) SET a += b` collapses two live pattern nodes (labels+properties; `aka` on the survivor). Agent-gated; find/`pin_map`/MERGE when \(|Q|>1\) still emit CueConflict and do not absorb. Distinct from ImportAbsorb (Path-B slice; SHALL NOT entity-resolve). Hidden handle stays off the wire. Not a product verb. SysML `SameThingAbsorb.implemented` stays **false** (Sysmler nest pin). Empty-q outline (0.11) SHALL NOT absorb. Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this absorb cut).

- **0.10 identity cut** — GraphElement is identity; hidden handle off the wire; optional `id` is a nickname. `CREATE ()` is legal. MERGE/SET/DELETE by labels+properties. `pin_map` from cue; CueConflict on find/`pin_map` emit when \(|Q|>1\). SCHEMA is not `id_first`. leftover NEW mint, leftover `require_anchor`, leftover `read_get`, ingest `allocate_from_locator` as PK, import MERGE-by-id, and leftover `id_policy keep|reject|remint` as product commands stay leftover (not taught). Do not claim **1.0**. Product `liveNeo4jClaimed` is **true** (0.14; not this identity cut). Neo4j adapter flush uses hidden-handle pattern MERGE (not `{id}`).
- **0.10 follow-up (hid off the product wire)** — Internal `_elN` / `hid` MUST NOT appear on CLI stdout, MCP emit, GQL ack, or jsonl. MERGE ack is labels+props / nickname only. MCP does not register leftover `read_get`. AgensGraph client MERGE uses `_memnet_hid` (same pattern as Neo4j; no live-Agens claim; server not vendored). leftover IdAllocator NEW mint and import remint tests are named leftover, not product contracts.
- **Cabinet hydrate MATCH by hid** — Neo4j / AgensGraph flush MERGE and hydrate ego MATCH share `_memnet_hid`. ABOUT / edges MATCH the same COM hid (not leftover nickname `COM_acme_live_neo4j` as the cabinet key). Official fixture flush then hydrate by the **same hid** that was written. leftover `id` is a property filter after hid miss only. Tests assert hid MERGE, not `MERGE (n:COM {id:`. Live 0.14 claim uses leftover-nickname hydrate after hid miss (see 0.14 note). Hidden handle stays off CLI/MCP/GQL emit.
...
- **leftover old extras numbering (not current law)** — historical plan row once listed 0.10 caller / 0.11 live Neo4j / 0.12 catalog Snap / 0.13 two namespaces / 0.14 HostSearch / 0.15 Peak_L / 0.16 export / 0.17 N-server. **Current** extras in package 0.19.0: 0.10 identity, 0.11 outline, 0.12 SameThingAbsorb, 0.13 caller, 0.14 live Neo4j claimed, 0.15 catalog Snap, 0.16 two namespaces, 0.17 HostSearch, 0.18 Peak_L, 0.19 export. N-server stays research [#47](https://github.com/chouswei/MemNet/issues/47). SSOT: [`docs/ROADMAP.md`](docs/ROADMAP.md).
- **Roadmap one picture** — [`docs/ROADMAP.md`](docs/ROADMAP.md): memory plane vs outer harness vs RAG vs cabinet; **1.0 = claim 0.5–0.8** (MAY tag from 0.9). Do not hold 1.0 for numbered extras.
- **Sessions as strata (design)** — partition haystacks into named sessions; goldfish one \(S\); join Absorb a slice. **SysML:** `Snap(one model) →` catalog + interiors (package grain). `ingest_sysml` 1→1 is as-is, not that Snap. Not Layer. [`docs/grammar/memnet-session-strata.md`](docs/grammar/memnet-session-strata.md).
...
- **MemNet ↔ Neo4j talk** — expand [`docs/grammar/neo4j-buffer.md`](docs/grammar/neo4j-buffer.md): what talks to what; key features on both sides; **RAG relatives** placed on Snap / Shape / cabinet. leftover design-era `liveNeo4jClaimed=false` in this bullet is leftover (product extra **0.14** claims true).
- **Version map locked (leftover snapshot of an older plan)** — 0.8 = GQL teach + shape for people; **0.9** = Neo4j client extra; 1.0 = 0.5–0.8 claimed. leftover Later-list in this bullet (Peak_L / HostSearch / N-server / export / live Neo4j as unclaimed) is leftover; current map is [`docs/ROADMAP.md`](docs/ROADMAP.md) (0.10–0.19 in package 0.19.0; N-server still research #47).
...
- **Goldfish I/O optimisation (design)** — one live `TSK` `pin_map`; leftover optional `view=shell` survey **on a seed** (not session outline); union-under-one-\(M\) leftover; sparse Δ. Cousins: PyG NeighborLoader, Graphiti `add_episode`, LightRAG dual-level grain, HiAgent replace. [#77](https://github.com/chouswei/MemNet/issues/77) note 28.
- **#77 research status after [#84](https://github.com/chouswei/MemNet/pull/84)** — notes 22–28 locked as design on `master`. Grammar status table: `docs/grammar/memnet-host-search-nest.md`. Leftover: HostSearchBridge ship. `Peak_L` is extra **0.18** (last-resort; never default goldfish).
...
- **Roadmap 0.5** — M1/M2/M3 done; leftover goldfish = 0.5.0. 0.6 honesty; 0.7 live cabinet. ACL / RSV / ingest are 0.4.x. Host search / GraphRAG stay out of the 0.5 engine.
...
### Added
- **Live AgensGraph cabinet proof** — `test_agens_live_flush_hydrate_round_trip` green against AgensGraph 2.16 when `MEMNET_AGENSGRAPH_URL` is set. `liveCabinetClaimed=true`. Server still not vendored; CI skips the live mark unless the URL is exported.

### Changed
- `AgensGraphAdapter`: autocommit after connect (flush must survive a later hydrate error); hydrate edges by id list among already-walked nodes (AgensGraph 2.16 `DuplicateAlias` / empty rematch).
- Version map: 0.5 goldfish; 0.6 honesty; **0.7 live cabinet**. Package **0.7.0**. Repo skill `memnet-reference` product **0.7.0**. Fake remains the always-on CI seam.

## [0.6.0] - 2026-08-18

### Added
- **V5 paradox** — five overlapping `pin_map` calls each prepend one `LAW01` (`test_v5_n_pin_maps_repeat_law`). Prefer one live `TSK`.
- **Offered durable** — process death = `session save` / `session load`. Playbook line in `docs/LLM-GUIDE.md`.
...
[Unreleased]: https://github.com/chouswei/MemNet/compare/v0.19.13...HEAD
[0.19.13]: https://github.com/chouswei/MemNet/compare/v0.19.12...v0.19.13
[0.19.12]: https://github.com/chouswei/MemNet/compare/v0.19.11...v0.19.12
[0.19.11]: https://github.com/chouswei/MemNet/compare/v0.19.10...v0.19.11
