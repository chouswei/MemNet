...
### Changed
- **Honesty `c` — skills: SSOT→code tracker; drop drifting code map** — Add `.cursor/skills/sysml-ssot-to-code/` (implementation ledger). Drop the duplicate `memnet-reference` code-map table (stale `cli/` path). Drop pack-only `sysml-allocate-generator` / hardware-generator teach from the relatives cache map (those folders are not vendored here). Route from `SKILL-GRAPH.md` / `AGENTS.md`. Same goldfish loop. No Hatch bump.
- **Honesty `c` — SSOT parts allocate to live code (one wheel, many hosts)** — `MemNetImplementation` `SoftwareAllocate` maps nested logical parts to Python modules in this checkout and **tracks implementation** (`ImplementationTracker`; derived map [`ssot-to-code-allocate-map.md`](sysml-models/outputs/ssot-to-code-allocate-map.md)). One Hatch `memnet-llm` wheel instantiates on droplet + devices (`oneWheelManyHosts`). MemNet MCP module stays tip/ops (`tipIsFace=false`). Cousin `sysmledge` is not in the wheel (`sysmlEdgeInWheel=false`; `CousinSysMLEdgeNotInRepo`; `mustNotInventUploadBind`; `sysmlEdgeTracked=false`). `MN-REQ-06.7` / `MN-VER-06-S04`. Engine/MCP behaviour unchanged. Not N-server (#47). Same goldfish loop. No Hatch bump. See [`sysml-models/outputs/ssot-to-code-allocate-case-study.md`](sysml-models/outputs/ssot-to-code-allocate-case-study.md).
- **Honesty `c` — device MemNet services, one MemNet MCP at the droplet (tip≠face)** — Ops fleet modelled outside `MemNetSystem`: `DropletHost` hosts the one `MemNetMcpServer` (MemNet tip/ops engine face; `tipIsFace=false`; `mcpCount=1`). Product invent face is cousin SysMLEdge (`sysmledge`; `mustNotInventUploadBind`). `MemNetDeviceHost[1..*]` run `MemNetCoreLibrary`; `mcpNested=false` means no **product** MCP nested; tip `memnet-mcp` on Pi for ops is legal (`tipMcpLegal=true`). Dual-MCP remeter: product Q → `sysmledge`; MemNet tip/ops → droplet MemNet MCP and/or Pi tip. `MN-REQ-06.6` / `MN-VER-06-S03`. Not N-server federation (#47). Same goldfish loop. No Hatch bump. See [`sysml-models/outputs/device-fleet-one-mcp-case-study.md`](sysml-models/outputs/device-fleet-one-mcp-case-study.md).
