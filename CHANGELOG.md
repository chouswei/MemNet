...
### Changed
- **RAG-relative algorithms** — [`docs/grammar/rag-relative-algorithms.md`](docs/grammar/rag-relative-algorithms.md): retrieve pipelines from source (GraphRAG local/global, LightRAG modes, Graphiti RRF \(k=1\) + 1-hop `node_distance`, HippoRAG PPR, RAGFlow weighted hybrid, mem0, Neo4j GraphQL `generate`). Placement stays [`docs/grammar/neo4j-buffer.md`](docs/grammar/neo4j-buffer.md); steal/reject stays the host-search nest / [#77](https://github.com/chouswei/MemNet/issues/77).
