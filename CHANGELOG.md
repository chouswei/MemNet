...
### Changed
- **RAG / Neo4j rethink (design)** — [`docs/grammar/memnet-neo4j-rag-rethink.md`](docs/grammar/memnet-neo4j-rag-rethink.md): as-is firewall pushes LLM↔Bolt; propose two ports on one Neo4j (cabinet hydrate vs library `RagHostHook`, disjoint namespace). Does not amend SHAPE; does not ship HostSearch. Reject fuse (RRF/PPR/`generate` on cabinet).
