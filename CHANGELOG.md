...
### Changed
- **0.12 SameThingAbsorb (untagged)** — Commit rule after CueConflict: `MATCH (a:Label {…}), (b:Label {…}) SET a += b` collapses two live pattern nodes (labels+properties; `aka` on the survivor). Agent-gated; find/`pin_map`/MERGE when \(|Q|>1\) still emit CueConflict and do not absorb. Distinct from ImportAbsorb (Path-B slice; SHALL NOT entity-resolve). Hidden handle stays off the wire. Not a product verb. SysML `SameThingAbsorb.implemented` stays **false** (Sysmler nest pin). Does not implement 0.11 outline engine (nest TARGET on master; empty cue still skips in the engine). Package SemVer remains **0.9.0**. Do not claim **1.0**. `liveNeo4jClaimed` stays false.
