...
### Changed
- **RAG-relative algorithms** — [`docs/grammar/rag-relative-algorithms.md`](docs/grammar/rag-relative-algorithms.md): retrieve pipelines from source. **No RAG hop** on `memnet-llm` ↔ Neo4j cabinet (`memnet-llm[neo4j]` is hydrate/flush only; host Snap is a third hop). Placement: [`docs/grammar/neo4j-buffer.md`](docs/grammar/neo4j-buffer.md). Steal/reject: host-search nest / [#77](https://github.com/chouswei/MemNet/issues/77).
