...
### Changed
- **Honesty `c` — SSOT parts allocate to live code (one wheel, many hosts)** — `MemNetImplementation` `SoftwareAllocate` maps nested logical parts to Python modules in this checkout. One Hatch `memnet-llm` wheel instantiates on droplet + devices (`oneWheelManyHosts`). MemNet MCP module stays tip/ops (`tipIsFace=false`). Cousin `sysmledge` is not in the wheel (`sysmlEdgeInWheel=false`; `CousinSysMLEdgeNotInRepo`; `mustNotInventUploadBind`). `MN-REQ-06.7` / `MN-VER-06-S04`. Engine/MCP behaviour unchanged. Not N-server (#47). Same goldfish loop. No Hatch bump. See [`sysml-models/outputs/ssot-to-code-allocate-case-study.md`](sysml-models/outputs/ssot-to-code-allocate-case-study.md).
