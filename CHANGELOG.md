...
### Added
- **Harness thesis (design)** — memory plane of a harness; few LLM tokens; relevant = Shape co-responds to \(q\); **many Flash-class calls**, not one stuffed generate. GitHub review: SWE-agent `LastNObservations`, OpenHands SDK condenser, Inspect `trim_messages`, Letta-code MemFS — transcript edit ≠ `pin_map`. [`docs/grammar/memnet-harness-thesis.md`](docs/grammar/memnet-harness-thesis.md).
