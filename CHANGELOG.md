...
### Changed
- **Honesty `c` — ops fleet tip≠face (#170) + SSOT→code allocate (#171)** — Tip SemVer so [#170](https://github.com/chouswei/MemNet/pull/170) and [#171](https://github.com/chouswei/MemNet/pull/171) can upload; PyPI already has **0.19.11** (2026-09-20). #170: one MemNet tip MCP at the droplet (`tipIsFace=false`; sysmledge is the product face; `mustNotInventUploadBind`). #171: `SoftwareAllocate` + `ImplementationTracker` (MN-REQ-06.7; one Hatch wheel, many hosts; sysmledge not in the wheel). Same goldfish loop. Numbered extras **0.10–0.19** unchanged. No 0.20 extra. No usage-method `b`. Hatch **0.19.12**; last published PyPI remains **`memnet-llm==0.19.11`** until upload. Do not claim tip=face. Do not invent a sysmledge product face in Hatch.
- **Package identity 0.19.12** — Hatch / `project.toml` / `memnet.__version__` honesty cut on **0.19**. Hatch is **0.19.12**; last published PyPI remains **0.19.11** until upload.
...
[Unreleased]: https://github.com/chouswei/MemNet/compare/v0.19.12...HEAD
