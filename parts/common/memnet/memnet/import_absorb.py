"""ImportAbsorb / ImportGuard — path B WorkingMemorySlice import (MN-REQ-12.9–12.11).

Lead-owned absorb of a bounded member slice into the mission session.
Product verb = import (not SessionMerge*; not micro merge=true).

Hard gates stay in the engine (schema/caps via store upsert, id policy, ACL
on lead mutate path). ImportGuard is an optional host soft-policy hook.
"""
...
# Optional process-wide host soft gate (MN-REQ-12.11). None = skip/disabled.
...
def normalize_id_policy(policy: str | None) -> IdPolicy:
    ...
    # Accept Cypher MERGE alias as keep/upsert-by-id.
    ...
    if text not in _ID_POLICY:
        raise MemNetError(
            ...
            "(keep = Cypher MERGE upsert-by-id)",
        )
...
def export_working_memory_slice(
    ...
    *,
    ...
    for anchor in anchors:
        ...
        for rec in rows:
            # LAW rows are session-local vocabulary — do not import as payload.
            if rec.tag == "LAW":
                continue
...
def absorb_working_memory_slice(
    ...
    *,
    ...
    """Hard-gate absorb into lead/mission session (MN-REQ-12.9 / 12.10).
    ...
    nodes = [r for r in work.records if r.tag != "EDG"]
