...
**Seed workflow:** `src/memnet/examples/workflow.rto-remote.example.txt` — **4 584 `@CMD` rows** extracted from the manual *List of commands* (rev 29). Regenerate with `python scripts/extract_rto_scpi.py` after placing the PDF at `data/rto/UserManual_en_29.pdf`.
...
Every agent turn follows the same loop (mirrors SysML and novel application notes):
...
```powershell
memnet session open --map-file src/memnet/examples/schema.techdocs.example.txt --ttl 10080
Get-Content src/memnet/examples/workflow.rto-remote.example.txt | memnet add --stdin --allow-new-relation
