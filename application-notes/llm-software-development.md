...
**Schema map:** `src/memnet/examples/schema.coding.example.txt`  
**Seed workflow:** `src/memnet/examples/workflow.coding.example.txt`
...
Every turn follows the same loop (aligned with novel, SysML, and tech-docs notes):
...
LAW-MEMNET01 is analogous to `LAW-PROSE*` in the novel note — other repos seed their own project LAW rows.
...
```powershell
memnet session open --map-file src/memnet/examples/schema.coding.example.txt --ttl 10080
Get-Content src/memnet/examples/workflow.coding.example.txt | memnet add --stdin --allow-new-relation
...
| `MOD_cli` | `src/memnet/cli.py` | CLI `session_save` / `session_load` (lines 339–363) |
| `MOD_mcp_server` | `src/memnet_mcp/server.py` | MCP wrappers (lines 100–125) |
| `MOD_mcp_client` | `src/memnet_mcp/client.py` | `run_memnet` bridge (line 128) |
...
| Id | Path | Line | Layer |
|----|------|------|-------|
| `SYM_cli_session_save` | `src/memnet/cli.py` | 339 | CLI Typer command |
| `SYM_cli_session_load` | `src/memnet/cli.py` | 351 | CLI Typer command |
| `SYM_mcp_session_load` | `src/memnet_mcp/server.py` | 100 | MCP `@mcp.tool()` |
| `SYM_mcp_session_save` | `src/memnet_mcp/server.py` | 120 | MCP `@mcp.tool()` |
| `SYM_run_memnet` | `src/memnet_mcp/client.py` | 128 | subprocess bridge |
...
In Python the seed is just `Path("src/memnet/examples/workflow.coding.example.txt").read_text(encoding="utf-8").splitlines()`. `@LAW` and `@EDG` are **built-in tags** — omit from `map_lines`; add via seed ingest or `add`. MCP auto-supplements core LAW01–LAW05 on open when absent.
...
add(wire_lines=[
  "@SYM: SYM_mcp_session_load|session_load|fn|src/memnet_mcp/server.py|100|async def session_load(...)|active|persistent",
  "@EDG: E_mcp_impl_load|SYM_mcp_session_load|implements|SYM_cli_session_load|wraps_cli|persistent"
])
...
| [`llm-novel-writer.md`](llm-novel-writer.md) | `session_load(keep_id=true)` resume pattern for long creative sessions |
...
memnet session open --map-file src/memnet/examples/schema.coding.example.txt
Get-Content src/memnet/examples/workflow.coding.example.txt | memnet add --stdin --allow-new-relation
...
- [`src/memnet/examples/schema.coding.example.txt`](../src/memnet/examples/schema.coding.example.txt)
- [`src/memnet/examples/workflow.coding.example.txt`](../src/memnet/examples/workflow.coding.example.txt)
