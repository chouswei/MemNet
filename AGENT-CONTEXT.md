...
Doctrine: **GQL only** + shaped **`pin_map`** from a cue + gated pattern Commit; leftover `id:'NEW'` mint is leftover. Handoff = **session id** (A→B pipe; B cue then `pin_map`; chat/HTTP/MissionDock never carry the graph). CapsPolicy **ACL shipped** when enabled; RSV + Path-B ingest shipped; 0.7 live cabinet proven (not vendored). Shape: `docs/SHAPE.md`. Pointers: `docs/grammar/gql-wire-profile.md` → `docs/LLM-GUIDE.md` → `docs/adr/ADR-001-gql-agent-wire.md`. Layer / Tier A archived — not product teach.
