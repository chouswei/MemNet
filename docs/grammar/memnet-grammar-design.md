...
| `docs/grammar/memnet-multi-layer.md` | **Design:** slim 1.x — NODE\|EDGE; law on node (`CST` + `ports=`/`law=`); EDGE = **bind** (`--(bind)-->` / `--bind--`); nesting = pin-map view; distinct from §3 I/O/store/transport layering |
