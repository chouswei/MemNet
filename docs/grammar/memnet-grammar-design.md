...
| `docs/grammar/memnet-field-formulas.md` | Field formulas as EDGE `derives` (design; no engine): MVP = one EDGE, `src_fields` **list** + `expr`, one `tgt_field` (multi-source same-node; not single-source-only) |
