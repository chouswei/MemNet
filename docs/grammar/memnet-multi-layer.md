...
- **GQL: consider / map, not teach as wire** — ISO Graph Query Language (property graphs; Cypher-like `MATCH`/`RETURN`) stays on the map for analogues and deliberate non-analogues. Crosswalk: [`gql-consideration.md`](gql-consideration.md). **MUST NOT** teach GQL as agent wire or as a peer dialect to Layer in 0.5.
