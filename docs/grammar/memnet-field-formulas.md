...
**Decision:** re-centre on **formula as EDGE relation** for the **flat same-node transitional MVP** only. Do **not** implement an expression engine yet. First concrete dialect candidate remains the **same-node self-loop** `derives` line with **`src_fields` as a multi-name list**. Keep `+=`/`-=` literal-only. **1.x ports-first (MemNet multi-layer §3.7):** laws live on **`FN`/`CST` nodes**; EDGE is a **carrier** between ports — do **not** treat formula-on-edge as the long-term teachable surface.
...
| `docs/grammar/memnet-multi-layer.md` | Capsules / stratified pin maps; **§3.7** — **`FN`/`CST` nodes hold the law**; EDGE = **carrier** (in/out), not the function/constraint |
