...
**Status:** identity SSOT. British English.  
...
1. **Named session \(S\)** — a labelled property graph (NODE | EDGE). The handle you pass is the **session id** (plus anchors / write scope). Peers **re-`pin_map`**. They do not receive a graph dump in chat.

2. **Goldfish read** — each turn the agent sees only a bounded **Recall Shape** \(\tilde{X} = \mathrm{Recall}(q)\), not raw \(S\). Cue first (id / kind / locator / keyword), then neighbourhood. Skip is valid when the seed is empty.
...
5. **Durable cabinet behind, not instead** — an AgensGraph-class store may hydrate/flush \(S\). It is not the handoff handle and not the default teach surface. Fake-alone is not a durable claim.
...
That is the shape. Versions **fill** it (leftover find, union-under-one-\(M\), live cabinet proof). They do **not** invent a different product.
...
- Claiming goldfish from `pin_map` alone when the leftover is LIMIT find + one \(M\) for multi-ego  
