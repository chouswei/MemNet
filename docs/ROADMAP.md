...
**Package now:** Hatch **0.19.7** (`memnet.__version__`). Last published PyPI wheel is **`memnet-llm==0.19.6`** until this cut is uploaded. Numbered extras **0.10–0.19** are in this package (unchanged). **0.19.7** honesty `c` mints leftover nicknames on catalog Snap PKG so snapshot save/load round-trips. GraphGlot parse front is on master (#109 @ 73a63c9b). Extra **0.14** claims `liveNeo4jClaimed=true`. **1.0** is still unclaimed (0.5–0.8).

**Last updated:** 2026-09-08 (`a.b.c` law locked; package **0.19.7** honesty `c` — catalog PKG leftover nicknames; extras **0.10–0.19** unchanged; Hatch **0.19.7**; last published PyPI **`memnet-llm==0.19.6`**; do not claim **1.0**; do not invent a **0.20** extra).
...
**0.19.1–0.19.7** were this (`session_close` + `sessions|n/max` + cap 1024 + ranking + nickname-off-`pin_map` + `SHAPE_DROP_KEYS` + Truncation-on-clip + catalog leftover nicknames shipped as `c` because the loop stayed the same).

Pure efficiency / speed on the current loop is **`0.19.c`**, not `0.20`.

### Consequences

- Package stays **0.19.7** until a later cut. Extras **0.10–0.19** stay the owns table below. **1.0** stays unclaimed.
...
| **0.10–0.19** | Numbered extras (table below). Each row is one `b` (usage-method revision). Same pattern as 0.9: **not** 1.0 gates | **Packaged** (Hatch **0.19.7**; last PyPI **0.19.6**; extras first shipped as 0.19.0) |
| **1.0.0** | **Claim** of **0.5 + 0.6 + 0.7 + 0.8**. Shape mature for people. Not GraphRAG. Not cabinet-only. Not a new engine. This is `a=1` | **Claim when coordinator tags** — package **0.19.7** does not claim 1.0 |
...
One concern per **`b`** (usage-method revision). Dependency order. **In package 0.19.0** (git tag by coordinator). Skip a `b` only if the coordinator writes the skip in CHANGELOG; do not fuse two usage-method changes into one `b`. Cuts on the same method are **`c`** (0.19.1–0.19.7). There is **no** 0.20 extra row.
