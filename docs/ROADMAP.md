...
- Package is **0.19.15**. Extras **0.10–0.19** stay the owns table below. **1.0** stays unclaimed.
...
One concern per **`b`** (usage-method revision). Dependency order. **In package 0.19.0** (git tag by coordinator). Skip a `b` only if the coordinator writes the skip in CHANGELOG; do not fuse two usage-method changes into one `b`. Cuts on the same method are **`c`** (0.19.1–0.19.15). There is **no** 0.20 extra row.
