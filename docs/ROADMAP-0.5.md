...
**Status:** version map below is the SemVer SSOT. Package on `master` is **0.7.0**. 0.8 is **not** shipped.  
...
| **0.8.0** | GQL-only **teach** + product **shape for people** ([#87](https://github.com/chouswei/MemNet/pull/87)). Docs / playbook / identity only. **No** engine cut. Rebase on 0.7 honesty — do not land a map that still says the cabinet is unclaimed | **Next** — not shipped |
...
**0.8 MUST NOT:** bump `memnet-llm` until the teach/shape cut lands; merge #87 dirty as-is; Peak_L / HostSearch / N-server / export / `rag_query`; second cabinet claim; vendor AgensGraph; restore Layer teach or novel-writer.
...
**Next SemVer:** **0.8.0** (table above). One-path gates below stay the teach/ops lock. Durable sketch: [`grammar/agensgraph-buffer.md`](grammar/agensgraph-buffer.md).
