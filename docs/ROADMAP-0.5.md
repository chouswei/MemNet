...
| **0.12.0** | **Catalog Snap** + **session strata**. List `session=` ids; look = `pin_map`; join = Absorb a **slice**. Teach: partition haystacks into sessions (mission / member / \(S_{\mathrm{lib}}\) / catalog), **not** Layer. [`grammar/memnet-session-strata.md`](grammar/memnet-session-strata.md). | Absorb shipped (0.4.x); 0.10 caller so catalog is not a dump | Rank sessions with ANN; Absorb a whole \(S\); Absorb Neo4j library nodes; Layer teach; one session per `TSK` |
