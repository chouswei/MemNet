...
**Dialect teach:** openCypher-shaped **GQL** + shaped `pin_map` + gated mutate — [`../grammar/gql-wire-profile.md`](../grammar/gql-wire-profile.md). Product **0.19.11.** Hatch **0.19.11**; last published PyPI is **`memnet-llm==0.19.6`**. **1.0** = 0.5–0.8 claimed (unclaimed; no extra engine). Playbook: [`../LLM-GUIDE.md`](../LLM-GUIDE.md). Honesty `c` (CueMiss/Peak_L vs CueConflict + Path-B SysML CON + snapshot leftover nick + `:inFile` + Truncation + `SHAPE_DROP_KEYS`): [`../operations/honesty-c-wire-audit.md`](../operations/honesty-c-wire-audit.md). Changelog: [`../../CHANGELOG.md`](../../CHANGELOG.md) **0.19.11**.  
...
| [`llm-sysml-v2-modeling.md`](system/llm-sysml-v2-modeling.md) | SysML model host; relatives of one cue; sub-unit in a **separate session**; MemNet is campaign memory | `schema.sysml.example.txt` **union** `schema.coding.example.txt` |
| [`llm-system-dev-multitask.md`](system/llm-system-dev-multitask.md) | Multitask in `modelbasedPrj-*` (git model SSOT; not SysMLEdge) / `SysMLEdgePrj-*` (bound desk when SysMLEdge-based) | sysml + coding (+ ingest maps as needed) |
