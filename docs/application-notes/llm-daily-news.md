...
**This file is one documented application example.** Use it as a template for batch LLM pipelines on MemNet: session-scoped working memory, hub metrics on minimal nodes, layered `@CLU`/`@SYN` summarisation, and prompt formatters as views over `query warm`. For shared-world multiplayer see `docs/application-notes/llm-mud.md`; for engine behaviour see `docs/LLM-GUIDE.md`.
