...
Not a long multi-stage creative pipeline — shorter and **no prose**:
...
**This file is one documented application example.** Use it as a template for LLM-flavoured MUDs on MemNet: shared world on the server, prose on the client, server agent for active rooms and ticks. For engine behaviour see `docs/LLM-GUIDE.md`.
