...
**Teach:** MemNet is **shared working memory**. Prefix is the model host — not a README flip. **`modelbasedPrj-*`** (and this engine checkout) is **repo-based**: git `sysml-models/` / `.sysml` is structural SSOT here (and of the code and docs it specifies). **MUST NOT** use SysMLEdge as model SSOT on that family. Token saving is two laws together: **relatives of one cue**, and each over-budget **sub-unit in a separate session**. Doctrine: [`memnet-session-strata.md`](../../extras/memnet-session-strata.md). Evidence: [`../../sysml-models/outputs/sysml-session-nest-cuts-case-study.md`](../../../sysml-models/outputs/sysml-session-nest-cuts-case-study.md). **`SysMLEdgePrj-*`** and bound: model graph on the bound desk; MemNet stays campaign goldfish — [`llm-system-dev-multitask.md`](llm-system-dev-multitask.md).
