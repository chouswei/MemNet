...
Optional-deps keep `pip install memnet-llm` lightweight; only `[mcp]` users pull the `mcp` package. Hatch **0.19.6**; published PyPI is **`memnet-llm==0.19.6`**: `pip install 'memnet-llm[mcp]'` (contributors: `pip install -e ".[mcp]"`).
