...
Optional-deps keep `pip install memnet-llm` lightweight; only `[mcp]` users pull the `mcp` package. Hatch **0.19.1**; last PyPI **`memnet-llm==0.19.0`** until upload: `pip install 'memnet-llm[mcp]'` (contributors: `pip install -e ".[mcp]"`).
