...
Optional-deps keep `pip install memnet-llm` lightweight; only `[mcp]` users pull the `mcp` package. Hatch **0.19.7**; last published PyPI is **`memnet-llm==0.19.6`**: `pip install 'memnet-llm[mcp]'` (contributors: `pip install -e ".[mcp]"`).
