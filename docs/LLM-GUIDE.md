...
| Mode | When | Setup |
|------|------|-------|
| **MCP in-process** | Cursor / local agents (**primary**) | Register `memnet-mcp` in `.cursor/mcp.json`; extra `[mcp]`. `pip install 'memnet-llm[mcp]'` (Hatch **0.19.5**; PyPI **0.19.5**). **No** `memnet serve` |
...
| Hard-truncate `pin_map` / Shape (drop FND / checklist / fundamentals) | Filter-out or raise `max_rows`; never claim a complete extract under a truncating window. Do not soften engine \(M\) |
