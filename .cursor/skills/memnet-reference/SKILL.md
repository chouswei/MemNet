...
**Product version:** `project.toml` / PyPI **`memnet-llm==0.3.3`** (CLI command remains `memnet`).
...
Shared dialect only — **never** `@RSV:` pipe. SSOT: `docs/grammar/memnet-neighbourhood-reserve.md`. Not implemented in 0.3.3.
...
**0.3.3:** Path A vs B remains valid. PinMapIngest_* may still be stubs; agents may seed pins via `session_open` `seed_lines` or `add` with explicit locator ids. Prefer ingest when available. SSOT: `docs/grammar/memnet-grammar-design.md` §4.2.1.
