...
Mint the stack with **`snap_model`** (catalog + interiors) or Path-B **`ingest_*`** into **one** current session (1→1 — that is not catalog Snap). `session_list` emits `sessions|n/max`; `session_close` that id (does not dump \(S\)) when a stratum is finished so later Snap can mint.
