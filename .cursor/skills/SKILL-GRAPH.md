...
This folder **vendors** the MemNet stack. Hatch **0.19.12**; last published PyPI is **`memnet-llm==0.19.11`**. Cue then `pin_map`; `find` if ego unknown. **GQL only.** Package SemVer `a.b.c` (claim / usage-method revision / same-method cut): `docs/ROADMAP.md`. Do not invent 0.20.
