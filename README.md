...
Package **`memnet-llm`** (CLI **`memnet`**). Python ≥ 3.11. Repo product **0.19.2**. Last published PyPI wheel is **`memnet-llm==0.19.0`** until `0.19.2` is uploaded (`pip install memnet-llm` still resolves 0.19.0). **1.0** stays unclaimed.

## Install + quick CLI

```bash
pip install memnet-llm
# or pin: pip install memnet-llm==0.19.2  (after PyPI upload; else ==0.19.0)
