...
def test_project_sysml_deterministic_locators(sysml_file: Path):
    ...
    assert "CREATE (:PKG" in gql or ":PKG" in gql
    assert "CREATE (:REQ" in gql or ":REQ" in gql
    assert "qname:" in gql or "qname: '" in gql
    assert "path:" in gql
    assert "NEW" not in gql
    assert "MERGE (n:" not in gql
...
def test_ingest_sysml_commit_and_pin_map(memnet_temp, sysml_schema: Path, sysml_file: Path):
    ...
    text = PinMapComposer(ss).compose(
        anchor=None,
        kind="REQ",
        locators=[("requirementId", "MN-REQ-DEMO.1")],
        depth=2,
    )[1]
    assert "REQ" in text or "requirementId" in text


def test_ingest_sysml_reingest_does_not_merge_by_id(
...
def test_project_codebase_deterministic_locators(code_file: Path):
    ...
    gql = "\n".join(result.gql_lines)
    assert "CREATE (:MOD" in gql or ":MOD" in gql
    assert "path:" in gql
    assert "line:" in gql
...
def test_ingest_codebase_commit_and_pin_map(memnet_temp, codebase_schema: Path, code_file: Path):
    ...
    assert result.committed
    mods = [r for r in ss.store._by_hid.values() if r.tag == "MOD"]
    syms = [r for r in ss.store._by_hid.values() if r.tag == "SYM"]
    assert mods
    assert syms
    ...
    assert defines
    assert calls
    text = PinMapComposer(ss).compose(anchor=mods[0].hid, depth=2)[1]
    assert "MOD" in text or "path" in text


def test_ingest_codebase_reingest_does_not_merge_by_id(
    memnet_temp, codebase_schema: Path, code_file: Path
):
    ss = open_session(map_file=str(codebase_schema))
    ingest_codebase(ss, code_file)
    n1 = len(ss.store._by_hid)
    ingest_codebase(ss, code_file)
    assert len(ss.store._by_hid) > n1
...
def test_ingest_pcba_commit_and_pin_map(memnet_temp, pcba_schema: Path, ato_file: Path):
    ...
    assert result.committed
    cmps = [r for r in ss.store._by_hid.values() if r.tag == "CMP"]
    nets = [r for r in ss.store._by_hid.values() if r.tag == "NET"]
    pins = [r for r in ss.store._by_hid.values() if r.tag == "PIN"]
...
def test_ingest_skills_commit_and_pin_map(memnet_temp, skills_schema: Path, skills_dir: Path):
    ...
    assert result.committed
    skls = [r for r in ss.store.list_records("SKL") if r.fields.get("skill_id") == "demo-ingest"]
    assert skls
    skl = skls[0]
...
def test_ingest_skills_reingest_does_not_merge_by_id(
    memnet_temp, skills_schema: Path, skills_dir: Path
):
    ss = open_session(map_file=str(skills_schema))
